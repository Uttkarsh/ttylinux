...
local installCmd="install --owner=0"

PKG_STATUS="Unspecified error -- check the ${PKG_NAME} build log"

cd "${PKG_NAME}-${PKG_VERSION}"
source "${TTYLINUX_XTOOL_DIR}/_xbt_env_set"
${installCmd} --mode=755  --group=0  chat/chat "${TTYLINUX_BUILD_DIR}/usr/sbin"
${installCmd} --mode=4550 --group=40 pppd/pppd "${TTYLINUX_BUILD_DIR}/usr/sbin"
source "${TTYLINUX_XTOOL_DIR}/_xbt_env_clr"
cd ..
