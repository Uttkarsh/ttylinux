...
[[ $# != 1 ]] && {
echo "This script takes one argument, the top-level ttylinux ISO directory."
echo "Usage: qemu-em-mipsel.sh <directory>"
exit 1
}
...
_path=""
for p in ${PATH//:/ }; do
	if [[ -x $p/qemu-system-mipsel ]]; then _path=$p/qemu-system-mipsel; fi
done
if [[ x"${_path}" = x ]]; then
	echo ""
	echo "Cannot find an executable \"qemu-system-mipsel\" program in your"
