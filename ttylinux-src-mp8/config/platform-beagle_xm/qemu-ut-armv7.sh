...
echo "Usage: qemu-sm-armv5tej.sh <directory>"
exit 1
}
...
_path=""
for p in ${PATH//:/ }; do
	if [[ -x $p/qemu-system-arm ]]; then _path=$p/qemu-system-arm; fi
done
if [[ x"${_path}" = x ]]; then
	echo ""
	echo "Cannot find an executable \"qemu-system-arm\" program in your"
