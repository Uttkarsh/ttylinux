...
# PROGRAM DESCRIPTION
#
#	This script builds the ttylinux root file system image.
#
# CHANGE LOG
#
...
echo ""
echo "=> Creating ttylinux file system image (${TTYLINUX_FSI_SIZE} MB)... "
rm --force "${TTYLINUX_IMG_NAME}"
dd if=/dev/zero of="${TTYLINUX_IMG_NAME}" bs=1M count=${TTYLINUX_FSI_SIZE}
...
find "${TTYLINUX_MNT_DIR}" -type d -o -type f \
	-exec touch --reference=${TTYLINUX_BUILD_DIR}/BUILD/TIME_STAMP {} \;
echo "DONE"
