...
# That was the last chance to stop before actually doing anything.
...
rm -f "${XBT_TARGET_DIR}/_log.1.binutils"
rm -f "${XBT_TARGET_DIR}/_log.2.gcc_stage1"
rm -f "${XBT_TARGET_DIR}/_log.3.kernel_headers"
rm -f "${XBT_TARGET_DIR}/_log.4.libc_stage1"
rm -f "${XBT_TARGET_DIR}/_log.5.gcc_stage2"
rm -f "${XBT_TARGET_DIR}/_log.6.libc_stage2"
rm -f "${XBT_TARGET_DIR}/_log.7.gcc_stage3"
rm -f "${XBT_TARGET_DIR}/_log.8.libc_stage3"
rm -f "${XBT_TARGET_DIR}/_log.9.target_adjust"
...
(
cd ${XBT_BUILD_DIR}
xbt_build_kernel_headers >${XBT_TARGET_DIR}/_log.1.kernel_headers 2>&1
xbt_build_binutils       >${XBT_TARGET_DIR}/_log.2.binutils       2>&1
...
if [[ $? -ne 0 ]]; then
	...
	elif [[ -f "${XBT_TARGET_DIR}/_log.1.kernel_headers" ]]; then
		echo "=> ${XBT_TARGET_DIR}/_log.1.kernel_headers"
	fi
	exit 1
fi
