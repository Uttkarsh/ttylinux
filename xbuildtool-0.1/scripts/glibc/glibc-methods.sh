...
# CHANGE LOG
#
#	31jan11	drj	Removed intemediate lib/crt[1in].o
...
#
# GLIBC header files are not installed.  The next GCC build will need them, so
# manually install them here.  Stubs.h may be an empty file, but GCC seems to
# build OK with it.
...
PATH=${XBT_XHOST_DIR}/usr/bin:${PATH} make csu/subdir_lib || exit 1
